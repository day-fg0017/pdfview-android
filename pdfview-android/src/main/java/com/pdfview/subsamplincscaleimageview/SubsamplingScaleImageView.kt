...
open class SubsamplingScaleImageView @JvmOverloads constructor(
    ...
    private var executor: Executor = AsyncTask.THREAD_POOL_EXECUTOR
    ...
    private fun setGestureDetector(context: Context) {
        this.detector = GestureDetector(context, object : SimpleOnGestureListener() {
            override fun onFling(
                ...
                if (panEnabled && isReady && vTranslate != null && e1 != null && (abs(
                        (e1.x - e2.x).toDouble()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (sWidth > 0 && sHeight > 0) {
            if (resizeWidth && resizeHeight) {
                ...
                height =
                    ((sHeight().toDouble() / sWidth().toDouble()) * width).toInt()
            } else if (resizeWidth) {
                width =
                    ((sWidth().toDouble() / sHeight().toDouble()) * height).toInt()
            }
        }
    ...
    private fun doubleTapZoom(sCenter: PointF?, vFocus: PointF?) {
        ...
        val doubleTapZoomScale =
            min(maxScale.toDouble(), doubleTapZoomScale.toDouble())
                .toFloat()
    ...
    private fun fitToBounds(center: Boolean, sat: ScaleAndTranslate) {
        ...
        if (panLimit == PAN_LIMIT_CENTER && isReady) {
            vTranslate.x =
                max(vTranslate.x.toDouble(), (width / 2 - scaleWidth).toDouble()).toFloat()
            vTranslate.y = max(vTranslate.y.toDouble(), (height / 2 - scaleHeight).toDouble())
                .toFloat()
        } else if (center) {
            vTranslate.x =
                max(vTranslate.x.toDouble(), (width - scaleWidth).toDouble()).toFloat()
            vTranslate.y =
                max(vTranslate.y.toDouble(), (height - scaleHeight).toDouble()).toFloat()
        } else {
            vTranslate.x = max(vTranslate.x.toDouble(), -scaleWidth.toDouble()).toFloat()
            vTranslate.y = max(vTranslate.y.toDouble(), -scaleHeight.toDouble()).toFloat()
        }
        ...
        val maxTy: Float
        if (panLimit == PAN_LIMIT_CENTER && isReady) {
            maxTx = max(0.0, (width / 2).toDouble()).toFloat()
            maxTy = max(0.0, (height / 2).toDouble()).toFloat()
        } else if (center) {
            maxTx = max(0.0, ((width - scaleWidth) * xPaddingRatio).toDouble())
                .toFloat()
            maxTy = max(0.0, ((height - scaleHeight) * yPaddingRatio).toDouble())
                .toFloat()
        } else {
            maxTx = max(0.0, width.toDouble()).toFloat()
            maxTy = max(0.0, height.toDouble()).toFloat()
        }

        vTranslate.x = min(vTranslate.x.toDouble(), maxTx.toDouble()).toFloat()
        vTranslate.y = min(vTranslate.y.toDouble(), maxTy.toDouble()).toFloat()
            ...
            try {
                ...
                if (context != null && decoderFactory != null && view != null) {
                    ...
                    if (view.sRegion != null) {
                        view.sRegion!!.left = max(0.0, view.sRegion!!.left.toDouble()).toInt()
                        view.sRegion!!.top = max(0.0, view.sRegion!!.top.toDouble()).toInt()
                        view.sRegion!!.right =
                            min(sWidth.toDouble(), view.sRegion!!.right.toDouble()).toInt()
                        view.sRegion!!.bottom =
                            min(sHeight.toDouble(), view.sRegion!!.bottom.toDouble()).toInt()
    ...
    private class Anim {
        ...
        var easing: Int = EASE_IN_OUT_QUAD // Easing style
        var origin: Int = ORIGIN_ANIM // Animation origin (API, double tap or fling)
        var time: Long = System.currentTimeMillis() // Start time
    ...
    private fun minScale(): Float {
        ...
        return if (minimumScaleType == SCALE_TYPE_CENTER_CROP || minimumScaleType == SCALE_TYPE_START) {
            max(
                ((width - hPadding) / sWidth().toFloat()).toDouble(),
                ((height - vPadding) / sHeight().toFloat()).toDouble()
            )
                .toFloat()
    ...
    private fun limitedScale(targetScale: Float): Float {
        ...
        targetScale = max(minScale().toDouble(), targetScale.toDouble()).toFloat()
        targetScale = min(maxScale.toDouble(), targetScale.toDouble()).toFloat()
        return targetScale
    }
    ...
    fun getPanRemaining(vTarget: RectF) {
        ...
        if (panLimit == PAN_LIMIT_CENTER) {
            vTarget.top = max(0.0, -(vTranslate!!.y - (height / 2)).toDouble())
                .toFloat()
            vTarget.left = max(0.0, -(vTranslate!!.x - (width / 2)).toDouble())
                .toFloat()
            vTarget.bottom = max(0.0, (vTranslate!!.y - ((height / 2) - scaleHeight)).toDouble())
                .toFloat()
            vTarget.right = max(0.0, (vTranslate!!.x - ((width / 2) - scaleWidth)).toDouble())
                .toFloat()
        } else if (panLimit == PAN_LIMIT_OUTSIDE) {
            vTarget.top =
                max(0.0, -(vTranslate!!.y - height).toDouble()).toFloat()
            vTarget.left =
                max(0.0, -(vTranslate!!.x - width).toDouble()).toFloat()
            ...
            vTarget.bottom = max(0.0, ((scaleHeight + vTranslate!!.y) - height).toDouble())
                .toFloat()
            vTarget.right = max(0.0, ((scaleWidth + vTranslate!!.x) - width).toDouble())
                .toFloat()
        }
    }
    ...
    fun setMinimumTileDpi(minimumTileDpi: Int) {
        ...
        val averageDpi = (metrics.xdpi + metrics.ydpi) / 2
        this.minimumTileDpi =
            min(averageDpi.toDouble(), minimumTileDpi.toDouble()).toInt()
        if (isReady) {
            reset(false)
            invalidate()
        }
    ...
    inner class AnimationBuilder {
        ...
        fun start() {
            ...
            val targetSCenter = if (panLimited) limitedSCenter(
                targetSCenter!!.x,
                targetSCenter.y, targetScale, PointF()
            ) else targetSCenter!!
            anim = Anim()
    ...
    companion object {
        private val TAG: String = SubsamplingScaleImageView::class.java.simpleName
        ...
        private val VALID_ORIENTATIONS: List<Int> = Arrays.asList(
            ORIENTATION_0,
            ORIENTATION_90,
            ORIENTATION_180,
            ORIENTATION_270,
        ...
        private val VALID_ZOOM_STYLES: List<Int> =
            Arrays.asList(ZOOM_FOCUS_FIXED, ZOOM_FOCUS_CENTER, ZOOM_FOCUS_CENTER_IMMEDIATE)

        /** Quadratic ease out. Not recommended for scale animation, but good for panning.  */
        const val EASE_OUT_QUAD: Int = 1

        /** Quadratic ease in and out.  */
        const val EASE_IN_OUT_QUAD: Int = 2

        private val VALID_EASING_STYLES: List<Int> = Arrays.asList(EASE_IN_OUT_QUAD, EASE_OUT_QUAD)
        ...
        const val PAN_LIMIT_INSIDE: Int = 1
        ...
        const val PAN_LIMIT_OUTSIDE: Int = 2
        ...
        const val PAN_LIMIT_CENTER: Int = 3

        private val VALID_PAN_LIMITS: List<Int> =
        ...
        const val SCALE_TYPE_CENTER_INSIDE: Int = 1
        ...
        const val SCALE_TYPE_CENTER_CROP: Int = 2
        ...
        const val SCALE_TYPE_CUSTOM: Int = 3
        ...
        const val SCALE_TYPE_START: Int = 4

        private val VALID_SCALE_TYPES: List<Int> = Arrays.asList(
        ...
        const val ORIGIN_ANIM: Int = 1

        /** State change originated from touch gesture.  */
        const val ORIGIN_TOUCH: Int = 2

        /** State change originated from a fling momentum anim.  */
        const val ORIGIN_FLING: Int = 3

        /** State change originated from a double tap zoom anim.  */
        const val ORIGIN_DOUBLE_TAP_ZOOM: Int = 4
